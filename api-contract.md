...
| `/api/track-story` | `user=<user>`, `track=<track id>`, optional `album=<album id>`, `artists=<csv artist ids>`, `releaseDate=<date or epoch ms>` | Progressive Bottom Track Story payload. | `counts`, `history.bestYear.{year,count,previousYearCount,nextYearCount}`, advanced metrics for tracks with more than 10 plays, `social`, and ordered `specialCards` (`shiny`, `hiddenGem`, `special`, `late`, `seasonal`). A `late` card exposes `value.{previousPlayedAt,returnedAt,gapDays}`. `seasonal` requires 3+ plays all in the same calendar month and that month recurring across at least two distinct years. Unproven counts are `null`; partial responses are not cached and must preserve existing modal fallbacks. Automatic flows must not add `force=1`. |
