...
| `/api/track-story` | `user=<user>`, `track=<track id>`, optional `album=<album id>`, `artists=<csv artist ids>`, `releaseDate=<date or epoch ms>` | Progressive Bottom Track Stats story payload. | `counts`, `history.bestYear.{year,count,previousYearCount,nextYearCount}`, advanced metrics for tracks with more than 10 plays, `social`, `specialCards`, and `coverage.partial|counts`. Unproven counts are `null`; partial responses are not cached and must preserve existing modal fallbacks. Automatic flows must not add `force=1`. |
