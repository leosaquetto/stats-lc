...
const normalizeTrack = (track: any) => {
  ...
  return {
    ...
    image: track.albumImage || track.album?.image || track.album?.images?.[0]?.url || track.image,
    album: track.album,
    albums: track.albums,
    ...
    albumImage: track.albumImage || track.album?.image || track.album?.images?.[0]?.url,
...
const normalizeRecentStream = (stream: any) => {
  ...
  const track = normalizeTrack({
    ...
    image: rawTrack.albumImage || stream.albumImage || rawTrack.album?.image || stream.album?.image || rawTrack.image || stream.trackImage,
    ...
    albumImage: rawTrack.albumImage || stream.albumImage || rawTrack.album?.image || stream.album?.image,
